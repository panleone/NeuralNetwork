...
public:
    ...
    void backward_internal(const Tensor<DType>& grad){
        ...
        if constexpr(std::is_same_v<Op, DApRELU> ){
            ...
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::VARIABLE_OP, ops::MUL_OP>>::eval(make_data_buffer<DType>(a_grad, a_res), a_grad);
        } else if constexpr(std::is_same_v<Op, DApLog>){
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::VARIABLE_OP, ops::DIVIDE_OP>>::eval(make_data_buffer<DType>(a_grad, a_res), a_grad);
        } else if constexpr(std::is_same_v<Op, DApFlipSign>){
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::FLIP_SIGN>>::eval(make_data_buffer<DType>(a_grad), a_grad);
        } else {
