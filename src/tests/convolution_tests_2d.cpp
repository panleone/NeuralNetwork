 ...
 */
static Tensor<double> naive_2d_convolution_forward(ConstTensor<double> kernel, ConstTensor<double> x, size_t STRIDE_HEIGHT, size_t STRIDE_WIDTH){
    ...
    const size_t EFFECTIVE_WIDTH  = (DATA_WIDTH  - KERNEL_WIDTH) / STRIDE_WIDTH + 1;
    const size_t EFFECTIVE_HEIGHT = (DATA_HEIGHT - KERNEL_HEIGHT) / STRIDE_HEIGHT + 1;
                    ...
                    for(size_t eff_w = 0; eff_w < EFFECTIVE_WIDTH; ++eff_w){
                        for(size_t ker_w = 0; ker_w < KERNEL_WIDTH; ++ker_w){
                            for(size_t ic = 0; ic < IN_CHANNELS; ++ic){
                                res(b, oc, eff_h, eff_w) += kernel(oc, ic, ker_h, ker_w)*x(b, ic, eff_h*STRIDE_HEIGHT + ker_h, eff_w*STRIDE_WIDTH + ker_w);
                            }
                        }
                    }
                }
 ...
 */
static std::pair<Tensor<double>, Tensor<double>> naive_2d_convolution_backward(
                    ...
                    for (size_t eff_w = 0; eff_w < EFFECTIVE_WIDTH; ++eff_w) {
                        for (size_t ker_w = 0; ker_w < KERNEL_WIDTH; ++ker_w) {
                            for (size_t ic = 0; ic < IN_CHANNELS; ++ic) {
                                ...
                                grad_kernel(oc, ic, ker_h, ker_w) += grad * x(b, ic, eff_h * STRIDE_HEIGHT + ker_h, eff_w * STRIDE_WIDTH + ker_w);
                                grad_x(b, ic, eff_h * STRIDE_HEIGHT + ker_h, eff_w * STRIDE_WIDTH + ker_w) += grad * kernel(oc, ic, ker_h, ker_w);
                            }
                        }
                    }
                }
...
static void convolution_operator_2d_tests(){
    ...
    for(size_t i = 0; i < test_runs; ++i){
        ...
        const size_t FEATURES_HEIGHT = random_size_t(KERNEL_HEIGHT, 100);
        const size_t FEATURES_WIDTH = random_size_t(KERNEL_WIDTH, 100);
        ...
        auto res = conv_2d(kernel, x_data).set_stride(STRIDE_HEIGHT, STRIDE_WIDTH);
        const auto& conv_parameters = res.get_parameters();
        he_initialization(conv_parameters);

        auto layer_res = res.forward();
        auto layer_res_simulated = naive_2d_convolution_forward(kernel.tensor, x_data.tensor, STRIDE_HEIGHT, STRIDE_WIDTH);

        if(!check_tensor_equality<double>(layer_res, layer_res_simulated, eps_threshold)){
            std::ostringstream oss;
            oss << "[CONV_2D_TEST]: forward pass error mismatch (actual, simulated)=(" << layer_res << ", " << layer_res_simulated << ")";
        ...
        const auto& [kernel_grad, x_grad] = naive_2d_convolution_backward(kernel.tensor, x_data.tensor, gradient, STRIDE_HEIGHT, STRIDE_WIDTH);
        res.backward(gradient);
        if(!check_tensor_equality<double>(kernel_grad, conv_parameters[0].gradient, eps_threshold)){
            std::ostringstream oss;
            oss << "[CONV_2D_TEST]: kernel gradient mismatch (actual, simulated)=(" << kernel_grad << ", " << conv_parameters[0].gradient << ")";
            throw std::runtime_error(oss.str());
        }

        if(!check_tensor_equality<double>(x_grad, conv_parameters[1].gradient, eps_threshold)){
            std::ostringstream oss;
            oss << "[CONV_2D_TEST]: x gradient mismatch (actual, simulated)=(" << x_grad << ", " << conv_parameters[1].gradient << ")";
            throw std::runtime_error(oss.str());
        }
    }
}