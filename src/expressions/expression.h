...
requires (HasToDexpr<A>) && (HasToDexpr<B>)
...
template<typename A, typename B>
requires (HasToDexpr<A>) && (HasToDexpr<B>)
auto matmul(const A& x, const B& y){
    return matmul(to_dexpr(x), to_dexpr(y));
}


template<typename A, typename B, typename C>
requires (HasToDexpr<A>) && (HasToDexpr<B>) && (HasToDexpr<C>)
auto conv_1d(const A& x, const B& y, const C& z){
    return conv_1d(to_dexpr(x), to_dexpr(y), to_dexpr(z));
}

template<typename A, typename B>
requires (HasToDexpr<A>) && (HasToDexpr<B>)
auto conv_2d(const A& x, const B& y){
    return conv_2d(to_dexpr(x), to_dexpr(y));
}

template<typename A>
requires (HasToDexpr<A>)
auto relu(const A& x){
    return relu(to_dexpr(x));
}

template<typename A>
requires (HasToDexpr<A>)
auto transpose(const A& x){
    return transpose(to_dexpr(x));
}

template<typename A>
requires (HasToDexpr<A>)
auto exp(const A& x){
    return exp(to_dexpr(x));
}

template<typename A>
requires (HasToDexpr<A>)
auto log(const A& x){
    return log(to_dexpr(x));
}

template<typename A>
requires (HasToDexpr<A>)
auto sqrt(const A& x){
    return sqrt(to_dexpr(x));
}


template<typename A>
requires (HasToDexpr<A>)
auto flatten(const A& x){
    return flatten(to_dexpr(x));
}
...
auto operator/(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApDivide>(
        static_cast<const A&>(x), static_cast<const B&>(y));
}

template<typename A, typename B>
auto operator*(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApMul>(
        static_cast<const A&>(x), static_cast<const B&>(y));
}

template<typename A, typename B>
auto operator+(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApSum>(
        static_cast<const A&>(x), 
        static_cast<const B&>(y));
}

template<typename A, typename B>
auto operator-(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApDiff>(
        static_cast<const A&>(x), 
        static_cast<const B&>(y));
}
...
auto matmul(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApMatMul<false, false>>(
        static_cast<const A&>(x),
        static_cast<const B&>(y)
    );
}
...
auto conv_1d(const DExpr<A>& x, const DExpr<B>& y, const DExpr<C>& z){
    return DTernExprOp<A, B, C, DApConv1d>(
        static_cast<const A&>(x),
        static_cast<const B&>(y),
        static_cast<const C&>(z)
    );
}

template<typename A, typename B>
auto conv_2d(const DExpr<A>& x, const DExpr<B>& y){
    return DBinExprOp<A, B, DApConv2d>(
        static_cast<const A&>(x),
        static_cast<const B&>(y)
    );
}
