...
public:
    MyModel(size_t out_channels) : l1{ ConvolutionLayer1D<DType>{1, out_channels, 3, 2}}, l2{ConvolutionLayer1D<DType>{out_channels, out_channels, 3, 2}},
                                 l3{ConvolutionLayer1D<DType>{out_channels, out_channels, 3, 2}}, l4{ LinearLayer<DType>(4*out_channels, 10)} {}
...
int main(){
    ...
    for(size_t epoch = 0; epoch < 6*50; epoch++){
        ...
        std::cout << "Epoch " << epoch << ", error rate: "<< 100.0f - static_cast<float>(good_preds) / static_cast<float>(total_preds) * 100.0f << "%" << std::endl;
    }
