...
class Shape{
    ...
    bool is_a_scalar;
public:
    Shape(const std::array<size_t, SHAPE_MAX_DIM>& in_shape, size_t in_dimension) : shape{in_shape}, dimension{in_dimension}{
    ...
    static bool are_compatible(const Shape& s1, const Shape& s2){
...
class GenericTensorData {
    private:
        ...
        template<typename> friend class GenericTensorData;
    public:
        T* data{nullptr};
        ...
        GenericTensorData(const GenericTensorData& t) :  data_size{t.data_size}, ref_counter{t.ref_counter}, data{t.data} {
            if(ref_counter){
                *ref_counter += 1;
            }
        }
        ...
        GenericTensorData(const GenericTensorData<K>& t) :  data_size{t.data_size}, ref_counter{t.ref_counter}, data{t.data} {
            if(ref_counter){
                *ref_counter += 1;
            }
        }

        GenericTensorData(GenericTensorData&& t) {
            swap(*this, t);
        }
        ...
        void set_constant(T x) const {
            std::fill(data, data + data_size, x);
        }
...
public:
    template<typename> friend class GenericTensor;

    GenericTensor(Shape in_shape) : shape{std::move(in_shape)}, avx_wrapped_size{get_avx_wrapped_size<T>(shape.get_size())}, tensor_data{avx_wrapped_size}{}

    GenericTensor(std::initializer_list<size_t> in_shape) : shape{in_shape}, avx_wrapped_size{get_avx_wrapped_size<T>(shape.get_size())}, tensor_data{avx_wrapped_size}{}
    ...
    template<typename K>
    requires (std::is_same_v<std::remove_const_t<T>, K>)
    GenericTensor(const GenericTensor<K>& t) : shape{t.shape}, avx_wrapped_size{t.avx_wrapped_size}, tensor_data{t.tensor_data}{}
    ...
    void set_zero() const {
        tensor_data.set_zero();
    }

    void set_constant(T x) const {
        tensor_data.set_constant(x);
    }
    ...
    T& operator()(std::initializer_list<size_t> idxs){
        ...
        bool are_indices_in_bound = std::equal(idxs.begin(), idxs.end(), shape.get_shape().begin(), [](size_t idx, size_t shape_idx){
            return idx < shape_idx;
        });
        assert(are_indices_in_bound);

        return operator[](std::inner_product(idxs.begin(), idxs.end(), shape.get_cumulative_shape().begin(), size_t{0}));
    }
...
public:
    ...
    TensorBroadcastableRef(ConstTensor<DType> tensor) : t_ref{std::move(tensor)}, t_size{tensor.get_size()} {
        t_ref.assert_ready_for_broadcasting();
    };
    const DType& operator[](size_t idx) const {
        return t_ref[idx % t_size];
    }
};
...
public:
    ...
    template<typename...T>
    requires (sizeof...(T) == N)
    DataBuffer(T&&... tensors) : expression_variables({TensorBroadcastableRef<DType>(std::forward<T>(tensors))...}) {};
