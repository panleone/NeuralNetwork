...
/**
 * Idea: In the backpropagation the neural network will output dl/dx, the derivative of the loss function with respect to the input vector.
 * We can check that this derivative is indeed the correct value, by using the approximated formula
 * dl/dx = (l(x + eps) - l(x-eps))/(2*eps)
 * for a small eps.
...
static void gradient_flow_tests(){
    ...
    for (int i = 0; i < 100; i++){
        ...
        check_num_equality(gradient, (resP - resM) / (2*eps), eps, "Backpropagation, gradient error");
