...
template<typename DType, size_t instruction, typename RegisterType, typename DataBuffer> inline void execute_instruction_avx(DataBuffer& data_pointers, RegisterType& registers, size_t i){
...
struct InterpretInternal;

template<typename DType, size_t...indices>
requires(std::is_same_v<DType, double> || std::is_same_v<DType, float>)
struct InterpretInternal<DType, Stack<indices...>>{
    ...
    static void eval(auto&& data_pointers, const Tensor<DType>& res) {
        ...
        constexpr size_t registers_stack_size = CountStack<Stack<indices...>, ops::VARIABLE_OP>::value;
...
inline void relu_backprop(Tensor<DType> input_grad, ConstTensor<DType> tensor){
    ...
    for(size_t i = 0; i < input_grad.get_size(); i+=avx_constants::intrinsic_size<DType>){
        ...
        simd_type<DType> cmp = _mm256_cmp_px<DType, _CMP_GT_OS>(v_tensor, avx_constants::zero<DType>);
...
inline DType get_max(ConstTensor<DType> t) {
    ...
    for(size_t i = avx_constants::intrinsic_size<DType>; i < t.get_size(); i+=avx_constants::intrinsic_size<DType>){
...
inline DType get_sum(ConstTensor<DType> t){
    ...
    size_t i = 0;
    for(; i + avx_constants::intrinsic_size<DType> <= t.get_size(); i+=avx_constants::intrinsic_size<DType>){
        simd_type<DType> v_t = _mm256_loadu_px(&t[i]);
...
static inline Tensor<DType> mat_mul_wrapper(const ConstTensor<DType>& t1, const ConstTensor<DType> & t2, const Shape& res_shape){
    ...
    blas_mat_mul<DType, transpose_t1, transpose_t2>(&t1[0], &t2[0], &res[0], t1_row, t1_col, t2_row, t2_col);
