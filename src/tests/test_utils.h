...
void should_throw(const std::function<void()>& fn, std::string_view reason){
    ...
    if (success) throw std::runtime_error(reason.data());
}

template<typename T>
void check_vector_equality(const Vector<T>& v1, const Vector<T>& v2, T eps, std::string_view reason){
...
template<typename T>
void check_matrix_equality(const Matrix<T>& m1, const Matrix<T>& m2, T eps, std::string_view reason){
