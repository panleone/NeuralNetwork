...
class DTernExprOp<A, B, C, DApConv1d> : public DExpr<DTernExprOp<A, B, C, DApConv1d>> {
public:
...
public:
    ...
    struct Simplify {
        using Type = DTernExprOp<typename A::Simplify::Type, typename B::Simplify::Type, typename C::Simplify::Type, DApConv1d>;
    };
    ...
    void compute_temporaries_for_eval() {
        ...
        auto kernel_matrix = kernel_im2col(Interpreter<typename SimplifiedT::Left>::const_interpret(a_), Interpreter<typename SimplifiedT::Right>::const_interpret(c_));
        auto x_data_matrix = x_im2col(Interpreter<typename SimplifiedT::Middle>::const_interpret(b_));
    ...
    void backward_internal(const Tensor<DType>& grad){
        ...
        auto [a_grad, c_grad] = kernel_col2im(mat_mul_wrapper<DType, true, false>(grad_im2col, x_data_im2col, kernel_data_im2col.get_shape()));
            ...
            for(size_t ic = 0; ic < IN_CHANNELS; ic++){
                for(size_t k = 0; k < KERNEL_SIZE; k++){
                    for(size_t w = 0; w < EFFECTIVE_WIDTH; w++){
                        tensor_im2col(b*EFFECTIVE_WIDTH + w, 1 + k + ic*KERNEL_SIZE) = tensor(b, ic, k + w * STRIDE);
                    }
                }
            }
        }
    ...
    std::pair<Tensor<DType>, Tensor<DType>> kernel_col2im(const ConstTensor<DType>& grad_matrix) const {
    ...
    Tensor<DType> x_col2im(const ConstTensor<DType>& grad_x_matrix) const {
        ...
        for(size_t b = 0; b < BATCH_SIZE; b++){
            for(size_t ic = 0; ic < IN_CHANNELS; ic++){
                for(size_t k = 0; k < KERNEL_SIZE; k++){
                    for(size_t w = 0; w < EFFECTIVE_WIDTH; w++){
                        grad_x(b, ic, k + w * STRIDE) += grad_x_matrix(b*EFFECTIVE_WIDTH + w, 1 + k + ic * KERNEL_SIZE);
                    }
                }
            }
        }
