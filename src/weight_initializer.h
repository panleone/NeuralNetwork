...
inline void he_initialization(const std::vector<Variable<DType, true>>& params) {
    for(const auto& param : params){
        ...
        if (t_dim == 0 || t_dim == 1){
            ...
            // TODO: not sure what to do for higher dimensional tensors ( > 4). Read some papers about it.
            assert(t_dim <= 4);
...
/**
 * For some tests we need outputs that are far from zero (becauese the RELU is non differentiable at zero).
 */
template<typename DType>
