 ...
 */
template <typename T>
requires(AreBinaryAndSimplifiable<T, LeftT<T>>) struct BinarySimplifier<T> {
    using ResOp = OperatorRulesT<OpT<T>, OpT<LeftT<T>>>;
    using Type = DTernExprOp<typename LeftT<LeftT<T>>::Simplify::Type,
                             typename RightT<LeftT<T>>::Simplify::Type,
                             typename RightT<T>::Simplify::Type,
                             ResOp>;
};
...
requires(!AreBinaryAndSimplifiable<T, LeftT<T>> &&
         AreBinaryAndSimplifiable<T, RightT<T>>) struct BinarySimplifier<T> {
    using ResOp = FlipRulesT<OperatorRulesT<OpT<T>, OpT<RightT<T>>>>;
    using Type = DTernExprOp<typename LeftT<T>::Simplify::Type,
                             typename LeftT<RightT<T>>::Simplify::Type,
                             typename RightT<RightT<T>>::Simplify::Type,
                             ResOp>;
};
...
requires(!AreBinaryAndSimplifiable<T, LeftT<T>> &&
         !AreBinaryAndSimplifiable<T, RightT<T>>) struct BinarySimplifier<T> {
    using Type =
        DBinExprOp<typename LeftT<T>::Simplify::Type, typename RightT<T>::Simplify::Type, OpT<T>>;
};
