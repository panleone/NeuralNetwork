...
public:
    StandardOptimizer(DType alpha, size_t batch_size, decltype(parameters)&& params) : alpha{alpha}, parameters{std::move(params)}{};
...
public:
    AdamOptimizer(DType alpha, DType beta, DType gamma, DType epsilon, decltype(parameters)&& params) : alpha{alpha}, beta{beta}, one_minus_beta{one - beta}, gamma{gamma}, one_minus_gamma{one - gamma}, epsilon{epsilon}, parameters{std::move(params)}{
    ...
    void optimize(size_t batch_size){
        ...
        for(size_t i = 0; i < parameters.size(); i++){
            ...
            momentum_sq += no_grad(one_minus_gamma) * (no_grad(gradient_norm)*no_grad(gradient_norm) - no_grad(momentum_sq));
...
private:
    ...
    std::vector<Tensor<DType>> momentums;
public:
    MomentumOptimizer(DType alpha, DType beta, decltype(parameters)&& params) : alpha{alpha}, one_minus_beta{one - beta}, parameters{std::move(params)}{
