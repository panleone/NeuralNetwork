            ...
            for (size_t j = 0; j < param.get_size(); j++) {
                ...
                if (std::abs(grad_increment) < tiny) {
                    if (std::abs(simulated_grad_increment) > 1e-8) {
                        ...
                        std::cout << i << std::endl;
                        oss << "[NN_TEST]: non-zero gradient (actual, simulated)=("
                            << grad_increment << ", " << simulated_grad_increment
                            << ") at iteration " << i;
                        throw std::runtime_error(oss.str());
                    }
                } else {
                    double rel_error =
                        std::abs((simulated_grad_increment - grad_increment) / grad_increment);
                    if (rel_error > 1e-5) {
                        std::ostringstream oss;
                        oss << "[NN_TEST]: gradient mismatch (actual, simulated)=("
                            << grad_increment << ", " << simulated_grad_increment
                            << ") at iteration " << i;
                        throw std::runtime_error(oss.str());
                    }
