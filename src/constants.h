...
template <typename T> struct struct_simd_type;
template <> struct struct_simd_type<float> {
  using type = __m256;
  static constexpr size_t size = 8;
};
template <> struct struct_simd_type<double> {
  using type = __m256d;
  static constexpr size_t size = 4;
};
template <typename T> using simd_type = typename struct_simd_type<T>::type;
...
namespace avx_constants{
    ...
    template<>
    constexpr simd_type<float> minus_one<float> = {-1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f};
