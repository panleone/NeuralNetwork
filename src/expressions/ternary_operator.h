...
requires (std::is_same_v<typename A::DType, typename B::DType> && std::is_same_v<typename B::DType, typename C::DType>) 
class DTernExprOp{
private:
    A a_;
    B b_;
...
public:
    ...
    struct Flatten {
        ...
        using tmp3 = std::conditional_t<recursive, typename C::template Flatten<true>::Type, Stack<ops::VARIABLE_OP>>;
    ...
    void get_parameters_internal(std::vector<std::pair<const Tensor<DType>&, const Tensor<DType> &>>& res) const {
