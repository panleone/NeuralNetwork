...
template <typename T>
concept IsMatmulOp = (std::is_same_v<OpT<T>, DApMatMul<false, false>>);
 ...
 */
template <typename T>
requires(IsMatmulOp<T> &&IsMatmulSimplifiable<T, LeftT<T>>
             &&IsMatmulSimplifiable<T, RightT<T>>) struct MatMulSimplifier<T> {
    using LeftOperand = typename LeftT<T>::Operand;
    using RightOperand = typename RightT<T>::Operand;
    using Type = DBinExprOp<typename LeftOperand::Simplify::Type,
                            typename RightOperand::Simplify::Type,
                            DApMatMul<true, true>>;
};
