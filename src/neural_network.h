...
template <typename T>
T lossFunction(const T& f,const T& y){
    return (f - y)*(f - y);
}

template <typename T>
T lossFunctionDerivative(const T& f,const T& y) {
    return 2*(f - y);
}
...
public:
    ...
    T forward(Vector<T> input, const T& out){
        ...
        std::for_each(nnLayers.begin(), nnLayers.end(), [&](auto& layer){ layer->forwardStep(input); });

        Vector<T> gradient{lossFunctionDerivative(input(0), out)};
        std::for_each(nnLayers.rbegin(), nnLayers.rend(), [&](auto& layer) {
    ...
    void backward(const T& alpha){
        std::for_each(nnLayers.begin(), nnLayers.end(), [&](auto& layer){ layer->finalize(alpha, batchedElements); });

        //std::cout <<  "[DEBUG] Average loss function is: " << accumulatedLoss/batchedElements << " on a batch of size: " <<  batchedElements << std::endl;
        //std::cout << "[DEBUG] Average gradient is: " << accumulatedGradient / static_cast<T>(batchedElements) << std::endl;
