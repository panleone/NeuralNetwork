...
static void check_matrix_equality(const Matrix<int>& m1, const Matrix<int>& m2, const std::string& reason){
    check_matrix_equality<int>(m1, m2, 0, reason);
}
static void check_vector_equality(const Vector<int>& v1, const Vector<int>& v2, const std::string& reason){
...
static void matrix_addition_tests(){
    ...
    Matrix<int> m_expected_diff{{-6, 18},
                                {-4, 34}};
...
static void transpose_tests(){
    ...
    Matrix<int> m1_transpose{{1, 4},
                             {2, 5},
                             {3, 6}};
    check_matrix_equality(m1.transpose(), m1_transpose, "Matrix transpose doesn't work");
    ...
    v1_transpose(0, 0)= 5;
    v1_transpose(0, 1)= 7;
    check_matrix_equality(v1.transpose(), v1_transpose, "Vector transpose doesn't work");
}
...
static void outer_product_tests(){
    ...
    check_matrix_equality(outerProduct(v1, v2), m, "Vector outer product doesn't work");
}
...
static void initializer_list_tests(){
    ...
    should_throw<MatrixException>([]{Matrix<int>{{1}, {1,2}};}, "Can create a matrix with different number of rows");
    should_throw<MatrixException>([]{Matrix<int>{{}, {}};}, "Can create a matrix with empty rows");
