...
class DBinExprOp : public DExpr<DBinExprOp<A, B, Op>>{
public:
...
public:
    ...
    struct Flatten{
        using tmp1 = std::conditional_t<recursive, typename A::template Flatten<true>::Type, Stack<ops::VARIABLE_OP>>;
        using tmp2 = std::conditional_t<recursive, typename B::template Flatten<true>::Type, Stack<ops::VARIABLE_OP>>;
    ...
    ConstTensor<DType> compute_temporaries_for_backprop() {
        if constexpr(!use_cache){
            ...
            res = InterpretInternal<DType, typename Flatten<false>::Type>::const_eval(make_data_buffer<DType>(a_res, b_res));
        }
        return res;
    }
    ...
    void backward_internal(const Tensor<DType>& grad){
        ...
        ConstTensor<DType> a_prev = a_.template compute_temporaries_for_backprop</*use_cache=*/true>();
        ConstTensor<DType> b_prev = b_.template compute_temporaries_for_backprop</*use_cache=*/true>();


        if constexpr(std::is_same_v<Op, DApSum> ){
            // Nothing to do
        } else if constexpr(std::is_same_v<Op, DApDiff>){
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::FLIP_SIGN>>::eval(make_data_buffer(b_grad), b_grad);
        } else if constexpr(std::is_same_v<Op, DApMul>) {
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::VARIABLE_OP, ops::MUL_OP>>::eval(make_data_buffer(a_grad, b_prev), a_grad);
            InterpretInternal<DType, Stack<ops::VARIABLE_OP, ops::VARIABLE_OP, ops::MUL_OP>>::eval(make_data_buffer(b_grad, a_prev), b_grad);
        } else {
